...
*/
// q: do we have different account manager for different accounts or its just one account manager that manages all accounts?
// a: one account manager that manages all accounts
contract AccountManager is ReentrancyGuard, Pausable, IAccountManager {
    ...
    */
    function borrow(
        ...
        // q: what is LToken ?
        // a: LToken e lending token
        // q: what is lending token ?
        // a: lending token e token koj se dobiva koga se pozajmuva nekoj asset
        // q: what are we checking here ?
        // a: we are checking if the token has a lending token
        // a: sekoj token moze da ima lending token t.e. Ltoken.sol t.e. tokenized vault contract so a brka cela rabota
